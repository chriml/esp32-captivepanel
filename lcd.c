...
/********************************************************************************/
				unsigned int code grayR[32]={0x0000,0x0800,0x1000,0x1800,0x2000,0x2800,0x3000,0x3800,0x4000,0x4800,0x5000,0x5800,0x6000,0x6800,0x7000,0x7800,
		   0x8000,0x8800,0x9000,0x9800,0xA000,0xA800,0xB000,0xB800,0xC000,0xC800,0xD000,0xD800,0xE000,0xE800,0xF000,0xF800};
		   
unsigned int code grayG[32]={0x0000,0x0040,0x0080,0x00C0,0x0100,0x0140,0x0180,0x01C0,0x0200,0x0240,0x0280,0x02C0,0x0300,0x0340,0x0380,0x03C0,
		   0x0400,0x0440,0x0480,0x04C0,0x0500,0x0540,0x0580,0x05C0,0x0600,0x0640,0x0680,0x06C0,0x0700,0x0740,0x0780,0x07C0};
		   
unsigned int code grayB[32]={0x0000,0x0001,0x0002,0x0003,0x0004,0x0005,0x0006,0x0007,0x0008,0x0009,0x000A,0x000B,0x000C,0x000D,0x000E,0x000F,
...
WriteComm(0xFE); WriteData(0x00);  //user command
//WriteComm(0x2A); WriteData(0x00);WriteData(0x04);WriteData(0x00);WriteData(0x7D);
//WriteComm(0x2B); WriteData(0x00);WriteData(0x00);WriteData(0x01);WriteData(0x25);
WriteComm(0x35); WriteData(0x01);	
WriteComm(0x36); WriteData(0x00);	//????
WriteComm(0x3A); WriteData(0x75); //16.7M???
WriteComm(0x51); WriteData(0xA0);  //????,??xff
WriteComm(0xC4); WriteData(0x80);	//set_DSPI Mode //moshi
...
void DispColor(unsigned int color)
{
	...
	BlockWrite(0,COL-1,0,ROW-1);
	  ...
	  CLKSEL = 0x03;
/*	
WriteComm(0x0036);WriteDispData(0x00,0xAF);
WriteComm(0x0037);WriteDispData(0x00,0x00);
WriteComm(0x0038);WriteDispData(0x00,0xDB);
WriteComm(0x0039);WriteDispData(0x00,0x00);
WriteComm(0x22);
*/
	CS0=0; 
	RS=1;
...
#define XSIZE  240		    //显示的行、列数
#define YSIZE  240

void DispGary0(void)	 
{
	unsigned char ii,jj,kk;
      unsigned int xsNum,xeNum,xxNum;
	unsigned int cc;
	...
	RS=1;	
      ...
      xxNum = XSIZE/16;
      xsNum = (XSIZE - xxNum*16)/2; 		//中间占用63灰阶
      xeNum = XSIZE - xxNum*16 - xsNum;
...
unsigned int code grayRB[32]={0x0000,0x0800,0x1000,0x1800,0x2000,0x2800,0x3000,0x3800,0x4000,0x4800,0x5000,0x5800,0x6000,0x6800,0x7000,0x7800,
		   0x8000,0x8800,0x9000,0x9800,0xA000,0xA800,0xB000,0xB800,0xC000,0xC800,0xD000,0xD800,0xE000,0xE800,0xF000,0xF800};
		   
unsigned int code grayGB[32]={0x0000,0x0040,0x0080,0x00C0,0x0100,0x0140,0x0180,0x01C0,0x0200,0x0240,0x0280,0x02C0,0x0300,0x0340,0x0380,0x03C0,
		   0x0400,0x0440,0x0480,0x04C0,0x0500,0x0540,0x0580,0x05C0,0x0600,0x0640,0x0680,0x06C0,0x0700,0x0740,0x0780,0x07C0};
		   
unsigned int code grayBB[32]={0x0000,0x0001,0x0002,0x0003,0x0004,0x0005,0x0006,0x0007,0x0008,0x0009,0x000A,0x000B,0x000C,0x000D,0x000E,0x000F,
		   0x0010,0x0011,0x0012,0x0013,0x0014,0x0015,0x0016,0x0017,0x0018,0x0019,0x001A,0x001B,0x001C,0x001D,0x001E,0x001F};




 void Disp_gray1(void)
{
	unsigned char ii,jj,kk;
      unsigned int ysNum,yeNum,yyNum;
	unsigned int cc;
	...
	RS=1;
      ...
      yyNum =  YSIZE/16;
      ysNum = (YSIZE - yyNum*15)/2; 		//中间占用63灰阶
      yeNum =  YSIZE - yyNum*15 - ysNum;
 
       
      cc = grayRB[0] | grayGB[0] | grayBB[0];
      for(ii=0;ii<ysNum;ii++)	      
      {
      	
		for(kk=0;kk<XSIZE;kk++) 
		{
			WriteData(cc>>8);
				WriteData(cc);
		}
      ...
      for(ii=1;ii<16;ii++)		
      {
            ...
            cc = grayRB[ii] | grayGB[ii] | grayBB[ii];
            for(jj=0;jj<yyNum;jj++)
            {
      ...
      cc = grayRB[16] | grayGB[16] | grayBB[16];
      for(ii=0;ii<ysNum;ii++)	      
      {
...
void disp_gray1(void)
{
	...
	RS=1;
      ...
      yyNum =  YSIZE/32;
      ysNum = (YSIZE - yyNum*30)/2; 		//中间占用63灰阶
      yeNum =  YSIZE - yyNum*30 - ysNum;
 
       
      cc = grayR[0] | grayG[0] | grayB[0];
      for(ii=0;ii<ysNum;ii++)	      
      {
...
void DispFrame(void)
{
	...
	{
		WriteData(RED>>8);WriteData(RED);
		for(i=0;i<COL-2;i++){WriteData(BLACK>>8);WriteData(BLACK);}
		WriteData(RED>>8);WriteData(RED);
	}

	WriteData(RED>>8);WriteData(RED);
	for(i=0;i<COL-2;i++){WriteData(0XFFFF);WriteData(0XFFFF);}
	WriteData(RED>>8);WriteData(RED);
	CLKSEL = 0x00;
	CS0=1;
}
...
void DispPicFromSD(unsigned char PicNum)
{
    ...
    BlockWrite(0,126-1,0,296-1);	 //COL  ROW

	CS0=0;
	RD0=1;
	RS=1;

	page_num=(unsigned long)293*(unsigned long)COL*2/512+1;	   //512
	page_s = PicNum*page_num+1;   	  		
//	page_e = (PicNum+1)*page_num+1;
	page_e=page_s+page_num;
    for(;page_s<page_e;page_s++)
    {  
	    MMC_SD_ReadSingleBlock(page_s-1);
